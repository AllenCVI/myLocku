...
import com.lockulockme.locku.module.ui.adapter.SheGiftAdapter;
import com.lockulockme.locku.module.ui.adapter.ShePicturesAdapter;
import com.lockulockme.locku.module.ui.adapter.SheTagAdapter;
import com.lockulockme.locku.module.ui.adapter.SheVideosAdapter;
...
public class SheDetailActivity extends BaseActivity<AcSheDetailBinding> {
    ...
    private SheTagAdapter sheTagAdapter;
    ...
    private void initView() {
        ...
        binding.rvTag.setLayoutManager(new LinearLayoutManager(mContext, RecyclerView.HORIZONTAL, false));
        sheTagAdapter = new SheTagAdapter();
        binding.rvTag.setAdapter(sheTagAdapter);
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.iv_msg:
                ...
                break;
            case R.id.tv_follow:
                followUser(userInfo.stringId);
                break;
            case R.id.tv_unfollow:
                unFollowUser(userInfo.stringId);
                break;
        }
    }

    private void followUser(String stringId) {
        if (TextUtils.isEmpty(stringId)) {
            return;
        }
        showLoading();
        AttentionHelper.getInstance().attentionUser(SheDetailActivity.this, stringId, new NewJsonCallback<Void>() {
            @Override
            public void onSuc(Void response, String msg) {
                ...
                EventBus.getDefault().post(new AttentionEvent(AttentionEvent.ATTENTION_TYPE, stringId));
            }
    ...
    private void unFollowUser(String stringId) {
        if (TextUtils.isEmpty(stringId)) {
            return;
        }
        showLoading();
        AttentionHelper.getInstance().unAttentionUser(SheDetailActivity.this, stringId, new NewJsonCallback<Void>() {
            @Override
            public void onSuc(Void response, String msg) {
                hideLoading();
                ...
                EventBus.getDefault().post(new AttentionEvent(AttentionEvent.UN_ATTENTION_TYPE, stringId));
            }
    ...
    private void userHello(String id) {
        VipManager.getInstance().getVipState(SheDetailActivity.this, new VipManager.OnVipListener() {
            @Override
    ...
    private void sendHi(String id) {
        OkGoUtils.getInstance().userSayHello(SheDetailActivity.this, new SheDetailsRequestBean(id), new NewJsonCallback<SayHelloResponseBean>() {
            @Override
    ...
    private void blockUser(String id) {
        showLoading();
        OkGoUtils.getInstance().blockUser(SheDetailActivity.this, new SheDetailsRequestBean(id), new NewJsonCallback<Void>() {
            @Override
            public void onSuc(Void response, String msg) {
                hideLoading();
    ...
    private void loadRealUserData(String id) {
        OkGoUtils.getInstance().getOtherInfo(SheDetailActivity.this, new SheDetailsRequestBean(id), new NewJsonCallback<UserInfo>() {
            @Override
            ...
            public void onE(int httpCode, int apiCode, String msg, UserInfo response) {
                ...
                binding.layoutCommonEmpty.rltCommonEmpty.setVisibility(View.GONE);
                binding.layoutCommonNetworkError.rltCommonNetworkError.setVisibility(View.VISIBLE);
                binding.slView.setVisibility(View.GONE);
            }
        });
        OkGoUtils.getInstance().getSheImages(SheDetailActivity.this, new SheImageRequestBean(id, 1, 50), new NewJsonCallback<List<SheImageResponseBean>>() {
            @Override
        ...
        OkGoUtils.getInstance().getSheVideos(SheDetailActivity.this, new SheImageRequestBean(id, 1, 50), new NewJsonCallback<List<SheVideoResponseBean>>() {
            @Override
        ...
        OkGoUtils.getInstance().getShePrices(SheDetailActivity.this, new VideoPriceRequestBean(id), new NewJsonCallback<List<AudioAndVideoPriceResponseBean>>() {
            @Override
        ...
        OkGoUtils.getInstance().getMacyList(SheDetailActivity.this, new SheMacyRequestBean(id), new NewJsonCallback<List<SheMacyResponseBean>>() {
            @Override
        ...
        OkGoUtils.getInstance().getSheGiftList(SheDetailActivity.this, new SheGiftRequestBean(id), new NewJsonCallback<SheGiftsResponseBean>() {
            @Override
        ...
        OkGoUtils.getInstance().getUserTags(SheDetailActivity.this, new SheTagRequestBean(id), new NewJsonCallback<SheTagResponseBean>() {
            @Override
            public void onSuc(SheTagResponseBean response, String msg) {
                fillStarData(response);
            }
        });
    }

    private void fillStarData(SheTagResponseBean response) {
        if (response == null) return;
        sheTagAdapter.setNewInstance(response.tags);
        DecimalFormatSymbols symbols = new DecimalFormatSymbols(Locale.US);
        DecimalFormat df = new DecimalFormat("0.#", symbols);
//        symbols.setDecimalSeparator('.');
//        df.setDecimalFormatSymbols(symbols);
        binding.ratingBar.setRating(Float.valueOf(df.format(response.floatScore)));
        binding.llStar.setVisibility(response.floatScore > 0 ? View.VISIBLE : View.GONE);
    }
...
//        }
        ...
        if (response.myLevelData != null && response.myLevelData.myLevel > 0) {
            binding.ivLevel.setVisibility(View.VISIBLE);
            GlideUtils.loadCircleImg(SheDetailActivity.this, response.myLevelData.LevelIcon, binding.ivLevel, R.mipmap.level_icon_place_holder);
        } else {
            binding.ivLevel.setVisibility(View.GONE);
        }

        binding.tvUnfollow.setVisibility(userInfo.followed ? View.VISIBLE : View.GONE);
        binding.tvFollow.setVisibility(userInfo.followed ? View.GONE : View.VISIBLE);
        binding.layoutCommonLoading.rltProgress.setVisibility(View.GONE);
        binding.layoutCommonEmpty.rltCommonEmpty.setVisibility(View.GONE);
        binding.layoutCommonNetworkError.rltCommonNetworkError.setVisibility(View.GONE);
        binding.slView.setVisibility(View.VISIBLE);
    }

    public void showPop() {
        final PopupWindowBuilder popBuilder = PopupWindowBuilder.newBuilder(SheDetailActivity.this)
    ...
    private void isEnough(String type, String id, UserInfo userInfo) {
        EnoughManager.getInstance().isEnough(type, id, SheDetailActivity.this, new EnoughManager.EnoughListener() {
            @Override
